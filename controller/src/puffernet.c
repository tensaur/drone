...
#define calloc(count, size) embedded_calloc(count, size)
...
void _linear(float* input, float* weights, float* bias, float* output, int batch_size,
...
void _linear_accumulate(float* input, float* weights, float* bias, float* output, int batch_size,
                        int input_dim, int output_dim) {
    for (int b = 0; b < batch_size; b++) {
        for (int o = 0; o < output_dim; o++) {
            float sum = 0.0f;
...
void _conv2d(float* input, float* weights, float* bias, float* output, int batch_size, int in_width,
             int in_height, int in_channels, int out_channels, int kernel_size, int stride) {
        ...
        for (int oc = 0; oc < out_channels; oc++) {
            for (int h = 0; h < h_out; h++) {
                for (int w = 0; w < w_out; w++) {
                    int out_adr =
                        (b * out_channels * h_out * w_out + oc * h_out * w_out + +h * w_out + w);
...
void _conv3d(float* input, float* weights, float* bias, float* output, int batch_size, int in_width,
             int in_height, int in_depth, int in_channels, int out_channels, int kernel_size,
             int stride) {
        ...
        for (int oc = 0; oc < out_channels; oc++) {
            for (int d = 0; d < d_out; d++) {
                for (int h = 0; h < h_out; h++) {
                    for (int w = 0; w < w_out; w++) {
                        int out_adr =
                            (b * out_channels * d_out * h_out * w_out + oc * d_out * h_out * w_out +
...
void _lstm(float* input, float* state_h, float* state_c, float* weights_input, float* weights_state,
           float* bias_input, float* bias_state, float* buffer, int batch_size, int input_size,
           int hidden_size) {
    _linear(input, weights_input, bias_input, buffer, batch_size, input_size, 4 * hidden_size);
    _linear_accumulate(state_h, weights_state, bias_state, buffer, batch_size, hidden_size,
                       4 * hidden_size);
...
void _embedding(int* input, float* weights, float* output, int batch_size, int num_embeddings,
                int embedding_dim) {
    for (int b = 0; b < batch_size; b++) {
...
void _layernorm(float* input, float* weights, float* bias, float* output, int batch_size,
                int input_dim) {
    for (int b = 0; b < batch_size; b++) {
        float mean = 0.0f;
        for (int i = 0; i < input_dim; i++) {
...
void _one_hot(int* input, int* output, int batch_size, int input_size, int num_classes) {
    for (int b = 0; b < batch_size; b++) {
        for (int i = 0; i < input_size; i++) {
            ...
            int out_adr = (b * input_size * num_classes + i * num_classes + input[in_adr]);
            output[out_adr] = 1.0f;
        }
    }
}
...
void _argmax_multidiscrete(float* input, int* output, int batch_size, int logit_sizes[],
                           int num_actions) {
    int in_adr = 0;
    for (int b = 0; b < batch_size; b++) {
...
void _softmax_multidiscrete(float* input, int* output, int batch_size, int logit_sizes[],
                            int num_actions) {
    int in_adr = 0;
    for (int b = 0; b < batch_size; b++) {
        for (int a = 0; a < num_actions; a++) {
...
void linear(Linear* layer, float* input) {
    _linear(input, layer->weights, layer->bias, layer->output, layer->batch_size, layer->input_dim,
            layer->output_dim);
}

void linear_accumulate(Linear* layer, float* input) {
    _linear_accumulate(input, layer->weights, layer->bias, layer->output, layer->batch_size,
...
Conv2D* make_conv2d(Weights* weights, int batch_size, int in_width, int in_height, int in_channels,
...
void conv2d(Conv2D* layer, float* input) {
    _conv2d(input, layer->weights, layer->bias, layer->output, layer->batch_size, layer->in_width,
            layer->in_height, layer->in_channels, layer->out_channels, layer->kernel_size,
            layer->stride);
}
...
Conv3D* make_conv3d(Weights* weights, int batch_size, int in_width, int in_height, int in_depth,
    ...
    size_t buffer_size =
        batch_size * out_channels * in_depth * in_height * in_width * sizeof(float);
...
void conv3d(Conv3D* layer, float* input) {
    _conv3d(input, layer->weights, layer->bias, layer->output, layer->batch_size, layer->in_width,
            layer->in_height, layer->in_depth, layer->in_channels, layer->out_channels,
...
void lstm(LSTM* layer, float* input) {
    _lstm(input, layer->state_h, layer->state_c, layer->weights_input, layer->weights_state,
          layer->bias_input, layer->bias_state, layer->buffer, layer->batch_size, layer->input_size,
          layer->hidden_size);
}
...
void embedding(Embedding* layer, int* input) {
    _embedding(input, layer->weights, layer->output, layer->batch_size, layer->num_embeddings,
               layer->embedding_dim);
}
...
void layernorm(LayerNorm* layer, float* input) {
    _layernorm(input, layer->weights, layer->bias, layer->output, layer->batch_size,
               layer->input_dim);
}
...
Default* make_default(Weights* weights, int num_agents, int input_dim, int hidden_dim,
                      int action_dim) {
...
LinearLSTM* make_linearlstm(Weights* weights, int num_agents, int input_dim, int logit_sizes[],
                            int num_actions) {
...
ConvLSTM* make_convlstm(Weights* weights, int num_agents, int input_dim, int input_channels,
                        int cnn_channels, int hidden_dim, int action_dim) {
    ConvLSTM* net = calloc(1, sizeof(ConvLSTM));
    net->num_agents = num_agents;
    net->obs = calloc(num_agents * input_dim * input_dim * input_channels, sizeof(float));
    net->conv1 =
        make_conv2d(weights, num_agents, input_dim, input_dim, input_channels, cnn_channels, 5, 3);
...
LinearContLSTM* make_linearcontlstm(Weights* weights, int num_agents, int input_dim,
    ...
    net->value_fn =
        make_linear(weights, num_agents, LSTM_DIM, 1); // 8-9. value (FIX: was LSTM_DIM+4)
