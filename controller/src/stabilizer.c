...
extern bool puffer_use_direct_motor_output;
...
void stabilizerInit(StateEstimatorType estimator) {
    ...
    STATIC_MEM_TASK_CREATE(stabilizerTask, stabilizerTask, STABILIZER_TASK_NAME, NULL,
                           STABILIZER_TASK_PRI);
...
static void batteryCompensation(const motors_thrust_uncapped_t* motorThrustUncapped,
    ...
    for (int motor = 0; motor < STABILIZER_NR_OF_MOTORS; motor++) {
        motorThrustBatCompUncapped->list[motor] =
            motorsCompensateBatteryVoltage(motor, motorThrustUncapped->list[motor], supplyVoltage);
    }
}
    ...
    while (1) {
        ...
        if (xSemaphoreTake(xRateSupervisorSemaphore, M2T(2000)) == pdTRUE) {
            ...
            if (isSensorsSuspended() == false) {
                ...
                ASSERT(false); // For safety, assert if the stabilizer task is blocking to ensure
                               // motor shutdown
            }
        }
    }
...
static void stabilizerTask(void* param) {
    ...
    STATIC_MEM_TASK_CREATE(rateSupervisorTask, rateSupervisorTask, RATE_SUPERVISOR_TASK_NAME, NULL,
    ...
    while (1) {
        ...
        if (healthShallWeRunTest()) {
            ...
            // Critical for safety, be careful if you modify this code!
            // The supervisor will already set thrust to 0 in the setpoint if needed, but to be
            // extra sure prevent motors from running.
            if (areMotorsAllowedToRun) {
                if (!puffer_use_direct_motor_output) {
...
/**
 * @brief Estimator type Auto select(0), complementary(1), extended kalman(2), **unscented kalman(3)
 * (Default: 0)
 *
...
/**
 * @brief Controller type Auto select(0), PID(1), Mellinger(2), INDI(3), Brescianini(4), Lee(5)
 * (Default: 0)
 */
...
/**
 * @brief The acceleration of the Crazyflie in the global reference frame, without considering
 * gravity, Z [Gs]
 */
...
/**
 * @brief The acceleration of the Crazyflie in the global reference frame, including gravity, Z
 * [mm/s]
 */
...
/**
 * @brief Requested motor power for m1, including battery compensation. Same scale as the motor PWM
 * but uncapped and may have values outside the [0 - UINT16_MAX] range.
 */
