      ...
      if (client->drone_model.meshCount > 0) {
        ...
        for (int p = 0; p < NUM_PROPELLERS; p++) {
          ...
          if (idx < client->drone_model.meshCount) {
            client->prop_centers[p] = compute_mesh_center(&client->drone_model.meshes[idx]);
          }
        }
...
void DrawDronePrimitive(Drone *agent, float *actions, Color body_color) {
  ...
  Vec3 rotor_offsets[4] = {{+diag, +diag, 0.0f},
                           {+diag, -diag, 0.0f},
                           {-diag, -diag, 0.0f},
                           {-diag, +diag, 0.0f}};
  ...
  for (int j = 0; j < 4; j++) {
    ...
    Color rotor_color = (Color){
        (unsigned char)(body_color.r * intensity),
...
void c_render(DroneEnv *env) {
  ...
  for (int i = 0; i < env->num_agents; i++) {
    ...
    if (norm3(agent->state.vel) > 0.1f) {
      ...
      DrawLine3D((Vector3){p.x, p.y, p.z}, (Vector3){p.x + v.x, p.y + v.y, p.z + v.z}, MAGENTA);
    }
    ...
    if (inspect_mode && is_selected) {
      ...
      DrawLine3D((Vector3){p.x, p.y, p.z}, (Vector3){t.x, t.y, t.z}, ColorAlpha(PUFF_GREEN, 0.5f));
    }
    ...
    if (trail->count > 2) {
      ...
      for (int j = 0; j < trail->count - 1; j++) {
        ...
        DrawLine3D((Vector3){p0.x, p0.y, p0.z}, (Vector3){p1.x, p1.y, p1.z}, ColorAlpha(trail_color, alpha));
      }
    }
  }
  ...
  DrawText(TextFormat("FPS: %d (W/S to adjust)", client->target_fps), 10, y, 18, WHITE); y += 22;
  if (client->model_loaded) {
    DrawText(TextFormat("Render: %s (M)", client->use_3d_model ? "3D Model" : "Primitive"),
             10, y, 18, client->use_3d_model ? PUFF_GREEN : LIGHTGRAY);
  }
  y += 30;
