...
void reset_agent(DroneEnv* env, Drone* agent, int idx) {
    ...
    set_target(&env->rng, env->task, env->agents, idx, env->num_agents, env->hover_target_dist);
    agent->ema_dist = norm3(sub3(agent->target->pos, agent->state.pos));
    agent->ema_vel = norm3(agent->state.vel);
    agent->ema_omega = norm3(agent->state.omega);
    agent->hover_ema = hover_score(agent);
